...
def snippets_list(request):
    ...
    return render(request, "snippets/list.html", {"snippets": qs, "q": q, "lang": lang})
...
def achievements(request):
    ...
    locked = [(code,Profile().BADGE_MAP.get((code if code in Profile.BADGE_MAP else code), (code, code, "🏆"))) for code in [c for c, *_ in ACHIEVEMENT_RULES] if code not in earned_codes]
    full_locked = [item for item in locked if isinstance(item[1], tuple)]
    return render(request, "achievements.html", {"earned": earned, "locked": full_locked})
